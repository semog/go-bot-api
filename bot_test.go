...
func ExampleNewBotAPI() {
	...
	botlog.Printf("Authorized on account %s", bot.Self.UserName)
	...
	for update := range updates {
		...
		botlog.Printf("[%s] %s", update.Message.From.UserName, update.Message.Text)
...
func ExampleNewWebhook() {
	...
	if info.LastErrorDate != 0 {
		botlog.Printf("failed to set webhook: %s", info.LastErrorMessage)
	}
